 ...
 */
char* pblSet_c_id = "$Id: pblSet.c,v 1.23 2009/03/11 23:48:44 peter Exp $";
 ...
 */
#define PBL_AVL_TREE_SET_LEFT( node, referenceNode )\
{\
    if( node->prev != referenceNode )\
        if(( node->prev = referenceNode )){ node->prev->parent = node; }\
}

#define PBL_AVL_TREE_SET_RIGHT( node, referenceNode )\
{\
...
)
{
    ...
    retptr = ( (PblHashSet*)set )->hashValue;
...
)
{
    ...
    return ( (PblHashSet*)set )->hashValue;
}
...
{
    PblTreeSet * this = pbl_malloc0( "pblSetNewTreeSet", sizeof(PblTreeSet) );
    if( !this )
    {
        return NULL;
    }

    this->genericSet.magic = PblTreeSetMagic;
...
{
    PblHashSet * this = pbl_malloc0( "pblSetNewHashSet", sizeof(PblHashSet) );
    if( !this )
    {
        return NULL;
    }

    this->genericSet.magic = PblHashSetMagic;
    this->hashValue = pblSetDefaultHashValue;
    this->loadFactor = 0.75;
    this->stepSize = _PBL_STEP_SIZE;
...
{
    PblTreeNode * newNode = pbl_malloc0( "pblTreeNodeClone",
                                         sizeof(PblTreeNode) );
    if( !newNode )
    {
        return newNode;
    ...
    if( node->prev )
    {
        ...
        PBL_AVL_TREE_SET_LEFT( newNode, clone );
    }
    ...
    if( node->next )
    {
        ...
        PBL_AVL_TREE_SET_RIGHT( newNode, clone );
    }
...
)
{
     ...
     */
    newSet->pointerArray = pbl_malloc0( "pblHashSetTrimToSize",
...
)
{
     ...
     * Malloc space for 'targetCapacity' pointers
     */
    newSet->pointerArray = pbl_malloc0( "pblHashSetEnsureCapacity",
                                         sizeof(void*) * targetCapacity );
    if( !newSet->pointerArray )
    {
        pblSetFree( (PblSet*)newSet );
...
{
    PblTreeNode * newNode = pbl_malloc0( "pblTreeNodeCreate", sizeof(PblTreeNode) );
    if( !newNode )
    {
        return newNode;
    }
...
PblTreeNode * parent,                  /** The parent node to insert into */
...
)
{
    ...
    *heightChanged = 0;
    ...
    if( compareResult < 0 )
    {
         ...
         */
        p1 = pblTreeNodeInsert( set, parent->prev, element, heightChanged );
        if( !p1 )
        {
            return p1;
        }
        PBL_AVL_TREE_SET_LEFT( parent, p1 );
        ...
        if( p1->balance == -1 )
        {
             ...
             * Simple LL rotation
             */
            PBL_AVL_TREE_SET_LEFT( parent, p1->next );

            PBL_AVL_TREE_SET_RIGHT( p1, parent );
            parent->balance = 0;
        ...
        PBL_AVL_TREE_SET_RIGHT( p1, p2->prev );

        PBL_AVL_TREE_SET_LEFT( p2, p1 );

        PBL_AVL_TREE_SET_LEFT( parent, p2->next );

        PBL_AVL_TREE_SET_RIGHT( p2, parent );
     ...
     */
    p1 = pblTreeNodeInsert( set, parent->next, element, heightChanged );
    if( !p1 )
    {
        return p1;
    }
    PBL_AVL_TREE_SET_RIGHT( parent, p1 );
    ...
    if( p1->balance == 1 )
    {
         ...
         * Simple RR rotation
         */
        PBL_AVL_TREE_SET_RIGHT( parent, p1->prev );

        PBL_AVL_TREE_SET_LEFT( p1, parent );
        parent->balance = 0;
    ...
    PBL_AVL_TREE_SET_LEFT( p1, p2->next );

    PBL_AVL_TREE_SET_RIGHT( p2, p1 );

    PBL_AVL_TREE_SET_RIGHT( parent, p2->prev );

    PBL_AVL_TREE_SET_LEFT( p2, parent );
...
PblTreeNode * parent,                       /** The parent node to balance     */
int  * heightChanged                        /** Set if the tree height changed */
)
{
    PblTreeNode * p1;
...
)
{
    ...
    resultArray = pbl_malloc( "pblSetToArray", sizeof(void*) * set->size );
    if( !resultArray )
    {
        return NULL;
    }
