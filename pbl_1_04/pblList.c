 ...
 */
char* pblList_c_id = "$Id: pblList.c,v 1.39 2009/03/11 23:48:44 peter Exp $";
...
{
    PblArrayList * this = pbl_malloc0( "pblListNewArrayList",
                                       sizeof(PblArrayList) );
    if( !this )
    {
        return NULL;
    ...
    this->genericList.magic = PblArrayListMagic;
...
{
    PblLinkedList * this = pbl_malloc0( "pblListNewLinkedList",
    ...
    this->genericList.magic = PblLinkedListMagic;
...
)
{
     ...
     */
    pointerArray = pbl_malloc( "pblArrayListEnsureCapacity", sizeof(void*)
            * minCapacity );
    if( !pointerArray )
    {
        return -1;
...
)
{
     ...
     */
    pointerArray = pbl_malloc( "pblListTrimToSize", nBytes );
    if( !pointerArray )
    {
        return -1;
    }
...
)
{
    ...
    newNode = pbl_malloc( "pblLinkedListAddAt", sizeof(PblLinkedNode) );
    if( !newNode )
    {
        return -1;
    }
...
)
{
    ...
    newNode = pbl_malloc( "pblLinkedListAdd", sizeof(PblLinkedNode) );
    if( !newNode )
    {
        return -1;
    }
...
)
{
    ...
    while( ( hasNext = pblIteratorHasNext( iterator ) ) > 0 )
    {
        ...
        newNode = pbl_malloc( "pblLinkedListAddAllAt", sizeof(PblLinkedNode) );
        if( !newNode )
        {
            // Out of memory,
            // remove the elements added so far
...
)
{
    ...
    this = (PblArrayList *)list;

    if( this->genericList.size + iteratorSize >= this->capacity )
    {
        /*
         * Make some more space
         */
        int capacity = this->capacity * 2 + 1;
        if( capacity < this->genericList.size + iteratorSize )
        {
            capacity = this->genericList.size + iteratorSize;
        }
        if( capacity < PBLAL_INITIAL_CAPACITY )
        {
            capacity = PBLAL_INITIAL_CAPACITY;
        ...
        if( pblArrayListEnsureCapacity( this, capacity ) < 0 )
        {
            return -1;
        }
    }

    if( index < this->genericList.size )
    {
        unsigned char * from = (unsigned char*)&( this->pointerArray[ index ] );
        unsigned char * to = from + iteratorSize * sizeof(void*);
        int length = sizeof(void*) * ( this->genericList.size - index );
    ...
    if( PBL_LIST_IS_ARRAY_LIST(collection) )
    {
        ...
        unsigned char * to = (unsigned char*)&( this->pointerArray[ index ] );
    ...
    else
    {
        while( ( hasNext = pblIteratorHasNext( iterator ) ) > 0 )
        {
            ...
            this->pointerArray[ index++ ] = (unsigned char *)next;
        }
        if( hasNext < 0 )
        {
            // Concurrent modification on the source collection
    ...
    this->genericList.size += iteratorSize;
    this->genericList.changeCounter++;
...
)
{
    ...
    if( toIndex < this->genericList.size )
    {
        unsigned char * to =
                (unsigned char*)&( this->pointerArray[ fromIndex ] );
        unsigned char * from = to + elementsToRemove * sizeof(void*);
        int length = sizeof(void*) * ( this->genericList.size - toIndex );
    ...
    {
        unsigned char * to =
                (unsigned char*)&( this->pointerArray[ this->genericList.size
                        - elementsToRemove ] );
        int length = sizeof(void*) * elementsToRemove;

        memset( to, 0, length );
        this->genericList.size -= elementsToRemove;
    }
...
)
{
    ...
    {
        PblArrayList * this = (PblArrayList *)list;
...
)
{
    ...
    {
        PblArrayList * this = (PblArrayList *)list;
        result = this->pointerArray[ index ];
        this->pointerArray[ index ] = element;
        return result;
    }
}
...
)
{
    ...
    resultArray = pbl_malloc( "pblLinkedListToArray", sizeof(void*)
            * list->genericList.size );
    if( !resultArray )
    {
        return NULL;
...
)
{
    ...
    else
    {
        ...
        return pbl_memdup( "pblListToArray", this->pointerArray, sizeof(void*)
                * this->genericList.size );
    }
}
